...
|  `_itoa.c`    |  converts an int into a string  |
|  `_putchar.c`    |  writes the character c to stdout |
|  `_strcat.c`    |  concatenates two strings |
|  `_strcmp.c`    |  compares two strings |
|  `_strcpy.c`    |  copies the string pointed to by src to dest |
|  `_strlen.c`    |  finds the length of the string |
|  `_strrev.c`    |  returns reversed string |
|  `_strtok.c`    |  finds and returns the tokens |
|  `env_handler.c`    |  handles the env built-in command |
|  `exit_handler.c`    |  takes care of `ctrl-D` |
|  `find_chars_number.c`    |  finds the number of characters c inside the string s, starting from the index idx |
|  `find_str_seq.c`    |  finds the index of the separator `sep` inside the string `s`, starting from the index `idx` |
|  `get_str_seq.c`    |  takes a sequence from the interval of s[beg:end],
 then copies it into the string passed by the pointer |
|  `insertion_sort.c`    |  Recursive function to sort an array using insertion sort |
|  `num_len.c`    |  returns the length of number |
|  `path_resolver.c`    |  resolves the PATH to a command `cmd` |
|  `print_env.c`    |  print out env |
|  `proc_handler.c`    |  creates and handles the child process |
|  `sig_handler.c`    |  takes care of signals |
|  `write_error.c`    |  writes the error msg to stdout |
|  `write_prompt.c`    |  writes the shell prompt |
